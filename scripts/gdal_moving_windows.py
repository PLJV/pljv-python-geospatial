...
if __name__ == "__main__":
    ...
    if not args["fun"]:
        try:
            ...
            raise KeyError(
                "user specified function can't be mapped to numpy "
                "or anything else : %s",
                e,
            )
    # -w/--window-size
    ...
    if not args["window_sizes"]:
        raise ValueError(
            "Moving window dimensions need to be specified using"
            "the -w argument at runtime. see -h for usage."
        )
    elif not args["raster"]:
        raise ValueError(
            "An input raster should be specified"
    ...
    if SHOW_PROGRESS:
        ...
        progress = manager.counter(
            total=len(_MATCH_ARRAYS), desc="Processing Moving Windows", unit="window"
        )
        ...
        for m in _MATCH_ARRAYS:
            ...
            if _MATCH_ARRAYS[m] is not None:
                ...
                focal.array = binary_reclassify(
                    array=focal.array, match=_MATCH_ARRAYS[m]
                )
            for w in args["window_sizes"]:
                f = str(args["outfile"] + "_" + str(w) + "x" + str(w))
                logger.debug(
                    "applying moving_windows.filter to reclassed "
                    "array for window size: %s",
                    w,
                )
                ndimage_filter(
                    {
    ...
    # otherwise just do our ndimage filtering
        ...
        for w in args["window_sizes"]:
            f = str(args["outfile"] + "_" + str(w) + "x" + str(w))
            logger.debug(
                "applying moving_windows.filter to image " "array for window size: %s",
                w,
            )
            output = copy(r)
            output.filename = f
            output.array = ndimage_filter(
                {"image": r, "function": args["fun"], "size": w, "dtype": args["dtype"]}
            )
            output.write()
            if SHOW_PROGRESS:
                progress.update()
