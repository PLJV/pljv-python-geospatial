    ...
    def __init__(self, json_conf=None, **kwargs):
        ...
        if json_conf is not None:
            with open(json_conf) as file:
                ...
                self.username = kwargs.get("username", json_conf.get("user", None))
                self.password = kwargs.get("password", json_conf.get("password", None))
                self.database_name = kwargs.get(
                    "database", json_conf.get("database", None)
                )
                self.table_name = kwargs.get("table_name", json_conf.get("table", None))
                self.crs = kwargs.get("crs", json_conf.get("crs", self.crs))
        else:
